"""
scripts/setup5_full.py
======================
ABLATION SETUP 5 - Full Architecture (ALL features ON)
Features: Vector search + BM25 + Neo4j graph + Dynamic Thresholds
          + Fact Checker + Intent Classifier

Run:
    cd "c:/LLM RAG/update promts"
    python scripts/setup5_full.py
"""
...
os.environ["USE_FACT_CHECKER"]        = "true"
os.environ["USE_INTENT_CLASSIFIER"]   = "true"

SETUP_NAME = "5_Full_Architecture"
...
print(f"  USE_DYNAMIC_THRESHOLDS=true | USE_FACT_CHECKER=true | USE_INTENT_CLASSIFIER=true")
print("=" * 70)
...
for i, item in enumerate(questions, 1):
    ...
    try:
        ...
        # Parse the structured research data into distinct contexts for RAGAS
        contexts = []
        if research_data:
            import re
            # Split the research_data by section headers to isolate each part
            parts = re.split(r'\n(?=Knowledge Graph Fact \d+\.|REVIEW EVIDENCE \(ChromaDB\):|RETRIEVAL NOTES:)', research_data)
            ...
            for part in parts:
                ...
                if part.startswith("Knowledge Graph Fact"):
                    kg_facts.append(part)
                elif part.startswith("REVIEW EVIDENCE"):
                    # This is the unmatched vector docs section. Split by the numbered list prefix (e.g. "1. ", "2. ")
                    import re
                    chunks = re.split(r'\n\d+\.\s+', "\n" + part)
                    for chunk in chunks:
                        if chunk.strip() and not chunk.strip().startswith("REVIEW EVIDENCE"):
                            contexts.append(chunk.strip())
                elif part.startswith("RETRIEVAL NOTES"):
...
metrics   = [Faithfulness(llm=ragas_llm), AnswerRelevancy(llm=ragas_llm, embeddings=ragas_emb), ContextPrecision(llm=ragas_llm), ContextRecall(llm=ragas_llm)]
...
meta_df   = pd.DataFrame([{"id": r["id"], "question": r["question"],
                            ...
                            "intent_confidence": r["intent_confidence"], "elapsed_seconds": r.get("elapsed_seconds", 0.0)}
                           for r in valid])
...
log("  USE_DYNAMIC_THRESHOLDS=true | USE_FACT_CHECKER=true | USE_INTENT_CLASSIFIER=true")
...
log(f"\n  Avg pipeline time: {sum(r.get('elapsed_seconds', 0.0) for r in raw_results)/len(raw_results):.1f}s" if len(raw_results) else "N/A")
